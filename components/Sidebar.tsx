    ...
    <>
      ...
      <div className="bg-white border border-gray-300 rounded p-3 shadow-sm">
        ...
        <div>
          ...
          >
            <option value="">{tables.length > 0 ? t.pleaseSelectTargetTable : t.pleaseConfigureConnection}</option>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            <div className="flex-1">
              <div className="font-medium text-[11px] text-gray-700">{t.createRecords}</div>
              <div className="text-[10px] text-gray-500">{t.createRecordsDesc}</div>
            </div>
          </label>
          <label
            className={`flex items-start gap-2 p-2 bg-white border rounded cursor-pointer transition-all ${
          ...
          >
            ...
            />
            <div className="flex-1">
              <div className="font-medium text-[11px] text-gray-700">{t.updateRecords}</div>
              <div className="text-[10px] text-gray-500">{t.updateRecordsDesc}</div>
            </div>
          </label>
        </div>
      </div>
      ...
      {mode === 'create' && (
        <div className="bg-white border border-gray-300 rounded p-3 shadow-sm">
          ...
          <div className="space-y-3">
            {/* Attachment Field */}
            <div>
              <label className="text-[10px] text-gray-500 mb-1 block">{t.attachmentField}</label>
              <select
              ...
              >
                {!targetTableId ? (
                  <option value="">{t.pleaseSelectTargetTableFirst}</option>
                ) : (
                  <>
                    <option value="">{attachmentFields.length > 0 || hasDefaultField ? t.selectAttachmentField : t.noAttachmentFieldAvailable}</option>
                    {attachmentFields.map(f => (
                      <option key={f.id} value={f.id}>
                        {f.name} {f.name === ATTACHMENT_FIELD_NAME ? `(${t.default})` : ''}
                      </option>
                    ))}
                    {!hasDefaultField && (
                      <option value="__create__">➕ {t.createTeaPrintResult}</option>
                    )}
                  </>
                )}
              </select>
              {!hasDefaultField && attachmentFieldId && attachmentFieldId !== '__create__' && (
                <p className="text-[10px] text-amber-600 mt-1">
                  {t.recommendTeaPrintResult}
                </p>
              )}
              {attachmentFieldId === '__create__' && (
                <p className="text-[10px] text-primary mt-1">
                  {t.willAutoCreateField}
                </p>
              )}
            </div>
            {/* Name Field */}
            <div>
              <label className="text-[10px] text-gray-500 mb-1 block">{t.nameField}</label>
              <select
                value={nameFieldId}
              ...
              >
                {!targetTableId ? (
                  ...
                  <>
                    <option value="">{textFields.length > 0 ? t.selectField : t.noFieldAvailable}</option>
      ...
      {mode === 'update' && (
        <div className="bg-white border border-gray-300 rounded p-3 shadow-sm">
          ...
          <div className="space-y-3">
            ...
            {/* Match Field */}
            <div>
              <label className="text-[10px] text-gray-500 mb-1 block">{t.matchField}</label>
              <select
                value={matchFieldId}
            ...
            {/* Match Type */}
            <div>
              <label className="text-[10px] text-gray-500 mb-1 block">{t.matchType}</label>
              <select
                value={matchType}
              ...
              >
                ...
                <option value="contains">{t.containsMatch}</option>
              </select>
            </div>
      ...
      <div className="bg-gray-100/50 border border-gray-200 rounded p-2.5">
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            className="w-3.5 h-3.5 accent-primary cursor-pointer"
          />
          <span className="text-[11px] text-gray-600">{t.skipUploadedFiles}</span>
        </label>
      </div>
    </>
  );
    ...
    <div className="bg-white border border-gray-300 rounded p-3 shadow-sm">
      ...
      <div className="space-y-2">
        <select
          ...
          className="w-full px-2 py-1.5 bg-white border border-gray-300 rounded text-[11px] text-gray-700 focus:outline-none focus:border-primary transition-all appearance-none bg-[url('data:image/svg+xml,%3Csvg%20xmlns%3D%27http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%27%20width%3D%2710%27%20height%3D%2710%27%20viewBox%3D%270%200%2024%2024%27%20fill%3D%27none%27%20stroke%3D%27%23999%27%20stroke-width%3D%272%27%3E%3Cpolyline%20points%3D%276%209%2012%2015%2018%209%27%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[right_8px_center] pr-6"
        >
          <option value="">{excelSheets.length > 0 ? t.selectWorksheet : t.pleaseUploadExcelFirst}</option>
        ...
        <select
          ...
          className="w-full px-2 py-1.5 bg-white border border-gray-300 rounded text-[11px] text-gray-700 focus:outline-none focus:border-primary transition-all appearance-none bg-[url('data:image/svg+xml,%3Csvg%20xmlns%3D%27http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%27%20width%3D%2710%27%20height%3D%2710%27%20viewBox%3D%270%200%2024%2024%27%20fill%3D%27none%27%20stroke%3D%27%23999%27%20stroke-width%3D%272%27%3E%3Cpolyline%20points%3D%276%209%2012%2015%2018%209%27%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-no-repeat bg-[right_8px_center] pr-6"
        >
          <option value="1">{t.headerRowOption} 1</option>
          <option value="2">{t.headerRowOption} 2</option>
          <option value="3">{t.headerRowOption} 3</option>
        </select>
        <label className="flex items-center gap-1.5 text-[11px] text-gray-600 pt-0.5">
          <input
            type="checkbox"
